...
// کش برای ذخیره وضعیت ربات‌ها و تریگرها
...
const releaseUserSingleInstance = async (userId) => {
  ...
  try {
    ...
    console.log(`🔗 ارسال درخواست تکی به: ${fullUrl}`);
...
bot.command('status', async (ctx) => {
  ...
  try {
    ...
    const cacheInfo = `\n💾 وضعیت کش: ${Math.round(cacheStats.keys / cacheStats.max * 100)}% پر`;
...
bot.on('text', async (ctx) => {
  ...
  try {
    ...
    console.error('خطا در پردازش پاسخ کارب��:', error);

    ctx.reply('❌ خطایی در پردازش رخ داد.');
